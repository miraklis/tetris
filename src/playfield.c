...
PlayField* createField(int wx, int wy, unsigned char playerNum, bool nextWindowPosToLeft)
{
    ...
    f->nextPieceWindow = createWindow(nextWindowX, wy, 6, 6, &palette.colorBlack, true);
    f->infoWindow = createWindow(wx, wy + FIELD_HEIGHT, FIELD_WIDTH, 6, &palette.colorBlack,true);
    ...
    moveText(f->scoreText, f->scoreText->base.x + (paddingX * 2), f->scoreText->base.y + (paddingY * 2));
    ...
    addObjectToWindow(f->infoWindow, (Object*)f->statusText);
    moveText(f->statusText, f->statusText->base.x + (paddingX * 2), f->statusText->base.y + (paddingY * 3));
    
    // Create the first tetrominoes
    ...
    addObjectToWindow(f->playWindow, (Object*)current);
    addObjectToWindow(f->nextPieceWindow, (Object*)next);
...
void getNextTetrominoe(PlayField* f)
{
    ...
    addObjectToWindow(f->playWindow, (Object*)f->currentPiece);
    ...
    addObjectToWindow(f->nextPieceWindow, (Object*)f->nextPiece);
...
void removeFilledLines(PlayField* f) 
{
    ...
    while(bottom > 0) {
        ...
        for(int i = 1; i < FIELD_WIDTH - 1; i++) {
            if(f->arena->layout[(bottom * FIELD_WIDTH) + i] != '=') {
                filled = false;
                break;
            }
        }
        if(filled) {
            ...
            while(yy > 1) {
                for(int i = 1; i < FIELD_WIDTH - 1; i++) {
                    f->arena->layout[(yy * FIELD_WIDTH) + i] = f->arena->layout[((yy - 1) * FIELD_WIDTH) + i];
                }
                yy--;
            }
            updateArena(f->arena);
...
int checkForFilledLines(PlayField* f, Tetrominoe* t)
{
    ...
    while(bottom > 0 && bottom >= top) {
        ...
        for(int i = 1; i < FIELD_WIDTH - 1; i++) {
            if(f->arena->layout[(bottom * FIELD_WIDTH) + i] == '*' 
                || f->arena->layout[(bottom * FIELD_WIDTH) + i] == '#') {
                    filled = false;
                    break;
            }
        }
...
bool checkGameOver(PlayField* f)
{
    ...
    int startRow = 0;
    for(int i = 1; i < FIELD_WIDTH - 1; i++) {
        if(f->arena->layout[(startRow * FIELD_WIDTH) + i] != '#') {
            isOver = true;
            break;
        }
    }
...
bool checkCollissions(PlayField* f, Tetrominoe* t, int offsetX, int offsetY, bool rotated)
{
    ...
    while(y < FIELD_HEIGHT && cnt < 4) {
        for(int i = 0; i < 4; i++)
            fieldMask[(cnt * 4) + i] = f->arena->layout[(y * FIELD_WIDTH) + fx + i];
        cnt++;
        y++;
    }
...
void addPieceOnStack(PlayField* f, Tetrominoe* t)
{
    ...
    for(int tc = 0; tc<16; tc+=4) {
        ...
        if(t->shapeLayout[tc + 0] != '.')
            f->arena->layout[(y * FIELD_WIDTH) + x + 0] = c;
        if(t->shapeLayout[tc + 1] != '.')
            f->arena->layout[(y * FIELD_WIDTH) + x + 1] = c;
        if(t->shapeLayout[tc + 2] != '.')
            f->arena->layout[(y * FIELD_WIDTH) + x + 2] = c;
        if(t->shapeLayout[tc + 3] != '.')
            f->arena->layout[(y * FIELD_WIDTH) + x + 3] = c;
        y++;
    }
    f->piecesCnt++;
}